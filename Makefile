...
$(gen_headers): header.h.jinja
	...
	$(eval u := $(call uc,$n))
	$(RENDER_JINJA) $< "include_guard=LIBVEC_VEC_$u_H" "sys_headers=libbase/$n.h" "rel_headers=" "type=$(or $($(n)_type),$n)" "typename=$n" "funcname=$n" "typeclasses=$(common_typeclasses) $($(n)_typeclasses)" -o $@
